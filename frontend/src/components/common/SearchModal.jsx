...
export function SearchModal({ open, onClose }) {
  ...
  const saveRecentTagQuery = (tag) => {
    ...
    const nextTags = [
      normalizedTag,
      ...recentTagQueries.filter((item) => item !== normalizedTag),
    ].slice(0, 6);
    setRecentTagQueries(nextTags);
    window.localStorage.setItem(
      "moodcast-recent-tags",
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      apiClient.get(`/posts/popular?limit=${POPULAR_POST_LIMIT}`),
      apiClient.get(`/posts`),
      apiClient.get(`/search/users/trending?limit=10`),
      apiClient.get(`/search/hashtags/trending?limit=10`),
    ])
      .then(
        ([popularPostsResult, latestPostsResult, usersResult, tagsResult]) => {
          if (popularPostsResult.status === "fulfilled") {
            setTrendingPosts(
              (popularPostsResult.value.data?.results || []).slice(
                0,
                POPULAR_POST_LIMIT,
              ),
            );
          ...
          if (latestPostsResult.status === "fulfilled") {
            setRecentPosts(
              (latestPostsResult.value.data?.results || []).slice(
                0,
                POPULAR_POST_LIMIT,
              ),
            );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(parsedTags)) {
        setRecentTagQueries(
          parsedTags.filter((tag) => typeof tag === "string" && tag.trim()),
        );
      }
    } catch {
      setRecentTagQueries([]);
    ...
    <div className={styles.overlay} onClick={onClose} role="presentation">
      ...
      >
        ...
        <div className={styles.tabs}>
          ...
          <button
            type="button"
            className={activeTab === "hashtags" ? styles.active : ""}
            onClick={() => setActiveTab("hashtags")}
            aria-pressed={activeTab === "hashtags"}
          >
            해시태그
          </button>
        </div>
          ...
          {query.trim() === "" ? (
              ...
              {activeTab === "hashtags" && (
                <div className={styles.trendingSection}>
                  ...
                  <div className={styles.quickTagSection}>
                    <h4 className={styles.quickTagTitle}>
                      {recentTagQueries.length > 0
                        ? "최근 검색한 태그"
                        : "실시간 트렌드 키워드"}
                    </h4>
                    <div className={styles.quickTagList}>
                      {(recentTagQueries.length > 0
                        ? recentTagQueries
                        : trendingTags.slice(0, 6).map((tag) => tag.hashtag)
                      ).map((tag) => (
                        <button
                          key={tag}
                          type="button"
            ...
            results.map((item) => {
                  ...
                  >
                    <div className={styles.itemHeader}>
                      ...
                      <span className={styles.itemCount}>
                        {(item.useCount ?? item.postCount ?? 0).toLocaleString(
                          "ko-KR",
                        )}
                      </span>
                    </div>
              ...
              return (
                ...
                >
                  <FeedCard post={transformPostData(item)} />
                </div>
              );
            })
          ) : (
